...
const packageCatalog: PackageInfo[] = [
  ...
  {
    name: "xserver-web",
    version: "X11 protocol",
    detail: "real in-browser X11 server with setup, windows, events, properties, and drawing",
    status: "loaded",
  },
  {
    name: "aurora-wm-web",
    version: "WM client",
    detail: "Aurora layout attached as an X11 window-manager client with SubstructureRedirect",
    status: "loaded",
  },
  {
    name: "xvfb",
    version: "not shipped",
    detail: "X11 display server; browser demo has no native display socket",
    status: "pending",
  },
  {
    name: "xserver",
    version: "not shipped",
    detail: "GUI server stack is outside the current headless WASM image",
    status: "pending",
  },
];
...
function packageList(installedPackages: ReadonlySet<string>): string {
  return [
    ...
    "xvfb              display       not packaged",
    "xserver           display       use startx / top button",
  ].join("\n");
}
...
function runCommand(
  ...
  if (["xvfb-run", "Xvfb", "xserver", "Xorg"].includes(command)) return { output: `${command}: command not found\nPackage status: not shipped in the current AgentOS WASM registry`, exitCode: 127, cwd, files: nextFiles };
