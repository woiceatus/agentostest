...
test("loads the Aurora layout target used by the X11 WM", async () => {
  ...
  const xserverBytes = await readFile(new URL("../public/wasm/xserver-web.wasm", import.meta.url));
  const xserver = (await WebAssembly.instantiate(xserverBytes, {})).instance.exports;
  assert.equal(typeof xserver.xserver_init, "function");
  assert.equal(typeof xserver.xserver_set_window, "function");
  xserver.xserver_init(960, 540);
  xserver.xserver_set_window(0, 24, 58, 400, 280, 1);
