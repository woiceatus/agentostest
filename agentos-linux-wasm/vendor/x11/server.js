...
class XServer extends EventEmitter {
    ...
    // Manual COMPOSITE redirection skips putting the redirected window into its
    // parent (client composites via NameWindowPixmap). Automatic still paints.
    compose() {
        const rootRaster = this.root.raster;
        const paint = (win, ox, oy) => {
            const subMode = win.compositeRedirectSubwindows;
            for (const child of win.children) {
                if (!child.mapped)
                    continue;
                const manual = child.compositeRedirect === 'Manual' || subMode === 'Manual';
                ...
                if (!manual) {
                    ...
                    // SHAPE bounding: blit each rect from the child raster.
                    const shape = child.shape && child.shape.bounding;
                    if (shape && shape.length) {
                        for (const r of shape) {
                            const sx = Math.max(0, r.x);
                            const sy = Math.max(0, r.y);
                            const sw = Math.min(r.width, child.width - sx);
                            const sh = Math.min(r.height, child.height - sy);
                            if (sw > 0 && sh > 0) {
                                const slice = child.raster.getPixels(sx, sy, sw, sh);
                                rootRaster.putPixels(null, cx + sx, cy + sy, sw, sh, slice);
                            }
                        }
                    } else {
