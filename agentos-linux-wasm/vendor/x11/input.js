...
function findButtonGrab(server, button, modMask) {
    const skip = server._replaySkipGrab || null;
...
function pointerFrozen(server) {
    const grab = server.grabs.pointer;
    return !!(grab && grab.frozen);
}

function activatePassiveButtonGrab(server, button, state, pg) {
    // pointerMode 0 = Sync (freeze until AllowEvents), 1 = Async
    const sync = pg.grab.pointerMode === 0;
    server.grabs.pointer = {
        client: pg.grab.client,
        wid: pg.win.id,
        ownerEvents: pg.grab.ownerEvents,
        eventMask: pg.grab.eventMask,
        cursor: pg.grab.cursor,
        implicit: true,
        frozen: sync,
...
function replayPointerButtonPress(server) {
    const grab = server.grabs.pointer;
    if (!grab || !grab.frozen)
        return;
    const button = grab.activateButton || 1;
    const state = grab.activateState != null ? grab.activateState : currentState(server);
    const skip = grab.passiveGrab || null;
    // Release the activating grab and replay as if it never occurred.
    server.grabs.pointer = null;
    server._replaySkipGrab = skip;
    try {
...
function install(server) {
    ...
    server.injectPointerMove = (x, y) => {
        ...
        server.pointer.x = x;
        server.pointer.y = y;
        // Sync grabs freeze delivery until AllowEvents; still update coords.
        if (pointerFrozen(server))
            return;
    ...
    server.injectButton = (button, isPress) => {
        ...
        if (isPress) {
            ...
            if (!server.grabs.pointer) {
                ...
                if (pg)
                    activatePassiveButtonGrab(server, button, state, pg);
            }
            const res = dispatchDeviceEvent(server, 4, button, eventMask.ButtonPress, false, state);
            if (!server.grabs.pointer && res) {
                const m = res.win.eventMasks.get(res.client) || 0;
            ...
            if (pointerFrozen(server)) {
                // Keep button state consistent even while frozen.
                server.pointer.buttons &= ~(1 << (button - 1));
                return;
            }
    ...
    h[26] = (client, ownerEvents, body) => {
        ...
        if (server.grabs.pointer && server.grabs.pointer.client !== client)
            ...
            server.grabs.pointer = {
                ...
                eventMask: mask, cursor, implicit: false, frozen: false
            };
    ...
    // AllowEvents (35) — Sync grab thaw / ReplayPointer for WM click-to-focus.
    // Modes: AsyncPointer=0 SyncPointer=1 ReplayPointer=2
    //         AsyncKeyboard=3 SyncKeyboard=4 ReplayKeyboard=5
    //         AsyncBoth=6 SyncBoth=7
    h[35] = (client, mode) => {
        const grab = server.grabs.pointer;
        if (mode === 2 /* ReplayPointer */) {
            if (grab && grab.client === client && grab.frozen)
                replayPointerButtonPress(server);
            return;
        }
        if (mode === 0 /* AsyncPointer */ || mode === 6 /* AsyncBoth */) {
            if (grab && grab.client === client)
                grab.frozen = false;
            return;
        }
        if (mode === 1 /* SyncPointer */ || mode === 7 /* SyncBoth */) {
            if (grab && grab.client === client)
                grab.frozen = true;
        }
    };
