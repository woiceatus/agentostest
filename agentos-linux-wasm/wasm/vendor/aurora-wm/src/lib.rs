...
#[cfg(feature = "web")]
pub(crate) type WmConn = x11rb::rust_connection::RustConnection<web_stream::JsStream>;
#[cfg(not(feature = "web"))]
pub(crate) type WmConn = RustConnection;

#[cfg(feature = "web")]
pub fn run() -> AnyResult<()> {
    Err("aurora-wm: native run() is unavailable in the web build; use aurora_wm_start/pump".into())
}
...
pub(crate) fn become_wm(conn: &WmConn, screen: &Screen) -> Result<(), ReplyError> {
...
fn wait_for_x_event_or_timeout(conn: &WmConn, timeout: Duration) {
    #[cfg(feature = "web")]
    {
        let _ = (conn, timeout);
        return;
...
pub(crate) fn init_light_compositor(conn: &WmConn, root: Window) -> bool {
...
pub(crate) fn disable_light_compositor(conn: &WmConn, root: Window) -> AnyResult<()> {
