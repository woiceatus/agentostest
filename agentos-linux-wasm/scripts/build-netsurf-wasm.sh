...
if command -v emcc >/dev/null 2>&1; then
  ...
  emcc "${src}" -O2 --no-entry \
    ...
    -Wl,--export=netsurf_init,--export=netsurf_frame_ptr,--export=netsurf_frame_len,--export=netsurf_width,--export=netsurf_height,--export=netsurf_is_running,--export=netsurf_render,--export=netsurf_address_buf,--export=netsurf_address_cap,--export=netsurf_commit_address,--export=netsurf_commit_title,--export=netsurf_clear_lines,--export=netsurf_line_buf,--export=netsurf_line_cap,--export=netsurf_add_line \
    -o "${out}"
else
